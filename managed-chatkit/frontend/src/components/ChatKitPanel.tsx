...
function VoiceButton() {
  ...
  // ✅ useRef to track current voiceState inside callbacks (stale closure fix)
  ...
  // Inject transcript into ChatKit composer
  const injectTextIntoChatKit = useCallback((text: string) => {
    ...
    if (!inputEl) {
      console.warn("[VoiceButton] ChatKit input field not found. Check selectors.");
      return;
    }
    ...
    if (inputEl instanceof HTMLTextAreaElement || inputEl instanceof HTMLInputElement) {
      ...
      if (nativeDesc?.set) nativeDesc.set.call(inputEl, text);
  ...
  useEffect(() => {
    ...
    recognition.lang = "en-US"; // Urdu ke liye: 'ur-PK'
    ...
    recognition.onend = () => {
      // ✅ voiceStateRef use karo — stale closure nahi hoga
    ...
    recognition.onerror = (e: SpeechRecognitionErrorEvent) => {
      ...
      if (e.error === "not-allowed") setStatusText("Mic permission do!");
      else if (e.error === "no-speech") setStatusText("Kuch suna nahi...");
    ...
    <>
      ...
      {statusText && (
        <div
          style={{
            ...
            right: 28,
            background:
              voiceState === "error" ? "#e24b4a" :
              voiceState === "recording" ? "#102b2d" : "#233a48",
            color: "#fff",
            fontSize: 13,
            padding: "5px 14px",
            borderRadius: 20,
      ...
      <button
        ...
        style={{
          ...
          background:
            voiceState === "recording" ? "#e24b4a" :
            voiceState === "processing" ? "#233a48" : "#102b2d",
          boxShadow:
  ...
  const options: ChatKitOptions = {
    ...
    theme: {
      ...
      color: {
        ...
        accent: { primary: "#233a48", level: 1 },
        surface: { background: "#102b2d", foreground: "#0e2025" },
      },
      typography: {
        baseSize: 16,
        fontFamily:
